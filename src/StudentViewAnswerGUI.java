...
/**
 * This class represents the interface where students view/see their answers and grades.
 */
public class StudentViewAnswerGUI extends JFrame {
    ...
    private JButton prevButton, nextButton, requestRegradeButton, exitButton;
    private ExamManager exManager;
    private AnswerManager ansManager;
    private RegradeManager reManager;
    private int question_no;

    public StudentViewAnswerGUI() {

        setTitle("Student View Answer GUI");
        ...
        //Code to calculate total grade
        totalScoreField.setText(totalGrade());
        ...
        exitButton.setBounds(400, 720, 150, 30);
    ...
    private String totalGrade() {
        int total_weight = 0;
        int total_grade = 0;
        for (int i = 1; i <= exManager.getLength(); i++) {
            total_weight += exManager.getQuestion(i).getWeight();
            int grade = ansManager.getAnswer(i).getGrade();
            if (grade == -1) {
                return "Ungraded";
            }
            total_grade += grade;
        }
        return String.format("%d / %d", total_grade, total_weight);
    }
}
